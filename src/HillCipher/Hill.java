...
public class Hill {
    ...
    public String[] runEncryption(String inputFile, String matrixFile) throws IOException {
        ...
        String line = plain.toString().toLowerCase();
        FileInputStream matrixRead = new FileInputStream(matrix);
        BufferedInputStream matrixIn = new BufferedInputStream(matrixRead);
        StringBuilder plainMatrix = new StringBuilder("");
        while (matrixIn.available() > 0){
            plainMatrix.append((char)matrixIn.read());
        }
        String key = plainMatrix.toString().toLowerCase();
    ...
    private String divide(String temp, int s) {
        String text = "";
        while (temp.length() > s) {
    ...
    private String perform(String line) {
        linetomatrix(line);
        linemultiplykey(line.length());
        String result = result(line.length());
        return result;
    }
    ...
    private String cofact(int num[][], int f) {
        int b[][], fac[][];
        b = new int[f][f];
        fac = new int[f][f];
        int p, q, m, n, i, j;
